...
Upon starting, the shell will load previously saved history from a file `singh.1790.history` if present. If history is not present it will be created from the current execution onwards. History of upto 10 commands will be stored in the shell. To view history enter `Ctrl+C`.
To run a command from history, enter `r x` where x is the first letter of the command from history.

History is saved to the file `singh.1790.history` upon exiting from shell.
NOTE : Please enter `Ctrl+Z` to exit the shell
