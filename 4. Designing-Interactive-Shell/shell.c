...
void write_queue_to_file() {
    FILE *fp;
    struct QNode* temp_ptr = q->front;

    fp = fopen("singh.1790.history", "a");//open for append
    if (fp == NULL){
        perror("singh.1790.history");
        exit(1);
    }
...
void read_queue_from_file() {
    FILE *fp;

    fp = fopen("singh.1790.history", "r");//open for read
    if (fp == NULL){
        fp = fopen("singh.1790.history", "w");
    }

    char read_buffer[25];
    while (fgets(read_buffer, sizeof(read_buffer), fp) != 0) {
        char* transfer_buffer = (char *)malloc(strlen(read_buffer)*sizeof(char));
        strncpy(transfer_buffer, read_buffer, strlen(read_buffer));
        int len = strlen(transfer_buffer);
        if (len > 0 && transfer_buffer[len-1] == '\n')
            transfer_buffer[len-1] = '\0';
        enQueue(q, transfer_buffer);
    }
    fp = fopen("singh.1790.history", "w");//open for append
    fclose(fp);
}
...
//        }
        ...
        if (inputBuffer[0] == command_search[0] && inputBuffer[1] == command_search[1]) {
            ...
            if (matched == 1) {
                printf("Command found in history! Executing searched command : %s\n", searched_command);
...
int main(void) {
    ...
    handler.sa_handler = handle_SIGSTP;
    ...
    while (1) {            /* Program terminates normally inside setup */
        background = 0;
        printf("Enter <Ctrl> + <Z> to Exit. COMMAND->");
        fflush(0);
